...
local function build_menu(modules)
    ...
    for _, cond in ipairs(conditions) do
        ...
        menu_setup.ui['send_button' .. cond] = menu_setup.register_ui(
            ui.new_button(tab[2], cont[2][1], 'Send', function() end),
    ...
    menu_setup.ui.fakelag_force = menu_setup.register_ui(
        ui.new_checkbox(tab[2], cont[2][2], COLORS.get("green", "ui") .. emojis.get("exlamation") .. COLORS.get("default", "ui") .. 'Force'),
    ...
    menu_setup.ui.fakelag_settings_freestanding = menu_setup.register_ui(
        ui.new_multiselect(tab[2], cont[2][2], 'freestanding options', 'static', 'zero pitch', 'defensive', 'side flip', 'pitch flip'),
    ...
    menu_setup.ui.fakelag_settings_enhance_onshot = menu_setup.register_ui(
        ui.new_multiselect(tab[2], cont[2][2], 'enhance on-shot aa', 'defensive', 'roll', 'jitter'),
    ...
    menu_setup.ui.fakelag_settings_antibrute = menu_setup.register_ui(
        ui.new_multiselect(tab[2], cont[2][2], 'antibrute', 'defensive', 'flip', 'roll', 'jitter'),
    ...
    menu_setup.ui.fakelag_settings_roll = menu_setup.register_ui(
        ui.new_slider(tab[2], cont[2][2], 'roll', 0, 45, 0, true, ''),
    ...
    menu_setup.ui.fakelag_settings_side = menu_setup.register_ui(
        ui.new_slider(tab[2], cont[2][2], 'side', 1, 3, 1, true, '', 1, {
    ...
    menu_setup.ui.paint_indicators = menu_setup.register_ui(
        ui.new_combobox(tab[2], cont[2][1], COLORS.get("green", "ui") .. emojis.get("pallete") .. COLORS.get("default", "ui") .. 'Indicators', 'off', 'small', 'bold'),
    ...
    menu_setup.ui.paint_logger = menu_setup.register_ui(
        ui.new_multiselect(tab[2], cont[2][1], COLORS.get("green", "ui") .. emojis.get("pen") .. COLORS.get("default", "ui") .. 'Logger', 'aimbot', 'config', 'anti-aim', 'other'),
...
local function update_visibility(modules)
	...
	if modules.menu_setup.ui.condition then
		...
		local paint_indicators_on = logged_in and tab_name == "PAINT" and ui.get(modules.menu_setup.ui.paint_indicators) ~= "off"
		local paint_indicators_small = logged_in and tab_name == "PAINT" and ui.get(modules.menu_setup.ui.paint_indicators) == "small"
		ui.set_visible(modules.menu_setup.ui.paint_indicators_animation, paint_indicators_on)
		ui.set_visible(modules.menu_setup.ui.paint_indicators_bar, paint_indicators_small)

		local paint_advertisement_on = logged_in and tab_name == "PAINT" 
		ui.set_visible(modules.menu_setup.ui.paint_advertisement_animation, paint_advertisement_on)

		local paint_watermark_on = logged_in and tab_name == "PAINT" 
		ui.set_visible(modules.menu_setup.ui.paint_watermark_type, paint_watermark_on)

		local paint_logger_selected = false
		if modules.menu_setup.ui.paint_logger then
			local ok, val = pcall(ui.get, modules.menu_setup.ui.paint_logger)
			if ok and val then
				if type(val) == 'table' then
					paint_logger_selected = (#val > 0)
				else
					paint_logger_selected = true
				end
			end
		end
		ui.set_visible(modules.menu_setup.ui.paint_logger_animation, logged_in and tab_name == "PAINT" and paint_logger_selected)
...
local function setup_callbacks(modules)
	...
	if modules.menu_setup.ui.paint_logger then
		ui.set_callback(modules.menu_setup.ui.paint_logger, function()
			update_visibility(modules)
		end)
	end
...
local function render_modern(x, y, w, h, alpha, text)
    local bg_x = x - 4
    local bg_y = y - 40
    local bg_w = w + 8
    local bg_h = h + 1
    ...
    draw_rounded_box(bg_x - 1, bg_y - 1, bg_w + 2, bg_h + 2, rad + 1, mr, mg, mb, out_a)
...
local function render()
    ...
    for i = #queue, 1, -1 do
        ...
        if time_left <= 0 then
            ...
            if style == 1 then
                render_modern(x, y, text_w, 13, alpha, log.text)
...
cb.register('paint', function()
    ...
    local speed = 12
    local step = speed * dt
    if M.abs(aspect_state.last - target) > 0.0005 then
        local t = M.clamp(step / M.max(M.abs(target - aspect_state.last), 0.01), 0, 1)
...
local function forward_from_angles(pitch, yaw)
...
cb.register('paint', function()
    ...
    for i=#tracers,1,-1 do
        ...
        if not f then
            ...
            local dir = forward_from_angles(ang)
            f = vector(s.x + dir.x * 8192, s.y + dir.y * 8192, s.z + dir.z * 8192)
        end
        local sx,sy = s:to_screen()
        local fx,fy = f:to_screen()
        if sx and fx then
...
cb.register('paint', function()
    ...
    elseif mode == 'sodium' then
        ...
        local elapsed = nowt - (frames.start or 0)

        
        local write_dur = 1.2
        local caps_hold = 1.8
        local lower_hold = 0.8
        local dewrite_dur = 1.2
        local cycle = write_dur + caps_hold + lower_hold + dewrite_dur
        local tmod = elapsed % cycle

        local tag = ""
        if tmod < write_dur then
            
            local progress = tmod / write_dur
            local chars = math.floor(progress * len + 0.5)
            if chars < 1 then chars = 1 end
            tag = target:sub(1, chars)
        elseif tmod < write_dur + caps_hold then
            
            local caps_elapsed = tmod - write_dur
            local flash_period = 0.6
            local on_time = 0.35
            if (caps_elapsed % flash_period) < on_time then
                tag = string.upper(target)
            else
                tag = target
            end
        elseif tmod < write_dur + caps_hold + lower_hold then
            
            tag = target
        else
            
            local dt = tmod - (write_dur + caps_hold + lower_hold)
            local progress = dt / dewrite_dur
            local chars = len - math.floor(progress * len + 0.5)
            if chars <= 0 then
                tag = ""
            else
                tag = target:sub(1, chars)
...
local kill_insults = {
    ...
    "₣łⱤɆ฿₳ⱠⱠ + ₴ØĐłɄ₥ = ₱ɆⱤ₣Ɇ₵₮łØ₦, ɎØɄⱤ ⱠɄ₳ = ₮Ɽ₳₴Ⱨ",
...
__bundle["require/features/paint/molotov_particles"] = [[
local ui = ui
local client = client
local entity = entity
...
callbacks.register('paint', function()
    ...
    local cam_pos = vector(camx, camy, camz)
    ...
    for _, ent in ipairs(infernos) do
        ...
        if ox then
            ...
            for _, off in ipairs(pts) do
                local pos = vector(ox + (off[1] or 0), oy + (off[2] or 0), oz + (off[3] or 0))
                local sx, sy = pos:to_screen()
                if sx and sy then
                    local dist = pos:distance_to(cam_pos)
...
local function draw_solus_watermark()
    ...
    local function build_perimeter(x,y,w,h,rad,step)
        ...
        local ex = right - rad
        if ex > sx then
            for xx = sx, ex, step do pts[#pts+1] = { xx, top } end
        end
        ...
        local ey = bottom - rad
        if ey > sy then
            for yy = sy, ey, step do pts[#pts+1] = { right, yy } end
        end

        add_arc_points(pts, right - rad, bottom - rad, rad, 0, 90, 6)

        if ex > sx then
            for xx = ex, sx, -step do pts[#pts+1] = { xx, bottom } end
        end

        add_arc_points(pts, left + rad, bottom - rad, rad, 90, 180, 6)

        if ey > sy then
            for yy = ey, sy, -step do pts[#pts+1] = { left, yy } end
        end
    ...
    local sample_step = 3
    local pts = build_perimeter(x-2, y-2, box_w+4, box_h+4, radius+2, sample_step)
    local n = #pts
    if n > 2 then
...
__bundle["require/help/bit"] = [[]]
__bundle["require/help/client"] = [[]]
...
return M]]
__bundle["require/help/renderer"] = [[]]
